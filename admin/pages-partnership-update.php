...
$idPartnership = $_POST['idPartnership'];
...
if (isset($_FILES["fileToUpload"]["tmp_name"]) && $_FILES["fileToUpload"]["error"] === UPLOAD_ERR_OK) {
    ...
    $target_file = $target_dir . $fotoPartnership;
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        // Update database
        $query = "UPDATE tb_partnership SET logo_partnership = '$fotoPartnership', nama_partnership = '$namaPartnership' WHERE id_partnership = '$idPartnership'";
        if (mysqli_query($conn, $query)) {
            header("Location: pages-partnership.php?pesan=update_success");
            exit;
        } else {
        ...
        header("Location: pages-partnership.php?pesan=update_fail");
        exit;
    }
} else {
    // Jika tidak ada berkas yang diunggah atau terdapat kesalahan
    $fotoPartnership = $hiddenImage;
